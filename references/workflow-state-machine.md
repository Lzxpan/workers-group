...
2. `KICKOFF`：五角色確認任務、範圍、分工、驗收與已知限制。
...
7. `QA`：QA 在 read-only 邊界獨立重跑並給出實際範圍的 verdict。
8. `BOSS_REVIEW`：Boss 比對 charter、QA evidence、授權與未驗證邊界。
...
標準前進路徑為 `INTAKE → KICKOFF → PLANNING → EXECUTING → EVIDENCE_REVIEW → QA → BOSS_REVIEW → CLOSED`。需要真人決策時進入 `AWAITING_HUMAN_APPROVAL`，取得明確決策後回到 `PLANNING` 或 `EXECUTING`。任一非結案工作若遇缺口可進入 `BLOCKED`；解除後回到 record 指定的 resume target。任一需要實際觀察但尚未執行的範圍可進入 `NOT_VERIFIED`；完成該觀察後回到對應的 `EXECUTING`、`EVIDENCE_REVIEW` 或 `QA`。`FAILED` 是失敗結案，`CLOSED` 只用於 evidence 支持的結案。

rework 不是 state 或 meeting type。當 QA verdict、evidence 或 Boss review 要求修正時，PM 建立新的 owned work item，將工作返回 `EXECUTING`，並保留原有 failure／verdict evidence。不得用重新實作覆蓋舊 evidence，也不得在尚未重跑 QA 前回到 `BOSS_REVIEW`。
