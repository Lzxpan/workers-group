...
from test_support import CODEX_HOME, ROOT, SCRIPTS, WorkersGroupTestCase
...
class HookContractTests(WorkersGroupTestCase):
    ...
    def test_pre_tool_use_denies_global_governance_file_writes(self):
        for target in (
            CODEX_HOME / "skills" / "orchestrating-workers-group" / "SKILL.md",
            CODEX_HOME / "agents" / "workers_qa.toml",
        ):
    ...
    def test_subagent_stop_validates_schema_task_role_commands_tests_and_repo_evidence(self):
        with self.temp_dir() as directory:
            ...
            (runtime / "active-task.json").write_text(
                json.dumps({"task_id": task_id, "status": "DONE"}),
                encoding="utf-8",
            )
            (root / "artifact.txt").write_text("verified", encoding="utf-8")
            valid = self._role_report(
                task_id, "workers_executor", "READY_FOR_QA", ["artifact.txt"],
            )
            result = self._run_hook(root, {
                "event": "SubagentStop",
    ...
    def test_qa_and_boss_completion_use_role_specific_reports_and_nested_evidence_gate(self):
        with self.temp_dir() as directory:
            ...
            boss = self._role_report(task_id, "workers_boss", "DONE", ["artifact.txt"])
    ...
    def test_planner_and_pm_completion_require_acceptance_validation_and_transition_record(self):
        with self.temp_dir() as directory:
            ...
            planner = self._role_report(
                task_id, "workers_planner", "READY", ["artifact.txt"],
            )
            pm = self._role_report(
                task_id, "workers_pm", "READY_FOR_BOSS_REVIEW", ["artifact.txt"],
            )
            ...
            pm_valid = self._run_hook(root, {
                ...
                "transitionRecord": {
                    ...
                    "to_status": "READY_FOR_BOSS_REVIEW",
                },
                "continuationCount": 0,
            })
    ...
    def test_stop_reports_each_exact_completion_gap_deterministically(self):
        with self.temp_dir() as directory:
            ...
            for fragment in (
                "NEEDS_REWORK", "acceptance_criteria", "failed_tests", "evidence",
    ...
    def test_session_start_integrity_bounded_queue_and_role_scoped_active_memory_retrieval(self):
        ...
        with self.temp_dir() as directory:
            ...
            (runtime / "active-task.json").write_text(
                json.dumps({"task_id": task_id, "status": "DONE", "objective": "hook memory"}),
                encoding="utf-8",
            )
