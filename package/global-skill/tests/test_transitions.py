...
class TransitionTests(WorkersGroupTestCase):
    def test_invalid_transition_is_rejected(self):
        ...
        with self.assertRaises(ValueError):
            module.validate_transition("INTAKE", "DONE", qa_verdict="PASS", evidence=["x"])

    def test_done_requires_pass_qa_and_readable_evidence(self):
        module = self.source_module("validate_transition.py")
        evidence = [str(Path(__file__).resolve())]
        with self.assertRaises((ValueError, PermissionError)):
            module.validate_transition(
                "READY_FOR_BOSS_REVIEW", "DONE", qa_verdict="FAIL", evidence=evidence,
            )
        with self.assertRaises((ValueError, PermissionError)):
            module.validate_transition(
                "READY_FOR_BOSS_REVIEW", "DONE", qa_verdict="PASS", evidence=[],
            )
    ...
    def test_human_waiver_must_be_structured_approved_and_have_readable_evidence(self):
        ...
        with self.assertRaises((ValueError, PermissionError)):
            module.validate_transition(
                "QA_REVIEW", "READY_FOR_BOSS_REVIEW",
    ...
    def test_done_requires_bound_valid_qa_boss_and_pass_acceptance_documents(self):
        module = self.source_module("validate_transition.py")
        flags = {
            "boss_reviewed": True,
