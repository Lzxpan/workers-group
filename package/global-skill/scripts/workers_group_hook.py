"""Shared dispatcher for globally installed Workers Group lifecycle Hooks."""
...
GAP_STATUSES = {
    "INTAKE", "CHARTERED", "PLANNING", "FEASIBILITY_REVIEW", "READY", "EXECUTING",
    "READY_FOR_QA", "QA_REVIEW", "READY_FOR_BOSS_REVIEW", "BLOCKED", "NEEDS_REWORK", "QA_FAILED",
...
COMPLETION_GAP_STATUSES = {"IN_PROGRESS", "BLOCKED", "NEEDS_REWORK", "QA_FAILED"}
ROLE_COMPLETION_STATUSES = {
    "workers_boss": {"DONE"},
    "workers_planner": {"FEASIBILITY_REVIEW", "READY"},
    "workers_pm": {"READY_FOR_BOSS_REVIEW", "DONE"},
    "workers_executor": {"READY_FOR_QA"},
}
...
def _completion_report_gaps(
    ...
    if event == "SubagentStop" and not role and not has_report:
        return []
    if event == "Stop" and not has_report and state.get("status") != "DONE":
        return []
    errors: list[str] = []
    if not task_id:
    ...
    if boss_report is not None and boss_report.get("status") != "DONE":
        errors.append("boss_report: final review status must be DONE")
    ...
    if status in COMPLETION_GAP_STATUSES:
        ...
        errors.append("status: final completion status must be DONE")
    errors.extend(_acceptance_errors(
        root,
...
def _has_report_gaps(payload: dict, state: dict) -> bool:
    ...
    if isinstance(report, dict):
        ...
        if status in {"READY_FOR_QA", "READY_FOR_BOSS_REVIEW", "DONE", "QA_PASSED"} and not report.get("evidence"):
            return True
