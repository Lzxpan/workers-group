...
def _cross_field_errors(
    ...
    completion = report.get("status") in {"READY_FOR_QA", "READY_FOR_BOSS_REVIEW", "DONE"}
    ...
    if kind == "role-report" and report.get("role") == "workers_executor" and report.get("status") == "READY_FOR_QA":
        for field in ("commands_run", "files_changed", "tests"):
            if not report.get(field):
                errors.append(f"$.{field}: Executor READY_FOR_QA requires recorded {field}")
        if report.get("blockers"):
            errors.append("$.blockers: Executor READY_FOR_QA cannot contain unresolved blockers")
    if kind == "role-report" and report.get("status") == "DONE":
        if report.get("blockers") or report.get("remaining_work"):
            errors.append("$: DONE role report cannot contain blockers or remaining work")
    if kind == "role-report" and report.get("role") == "workers_planner" and report.get("status") == "FEASIBILITY_REVIEW":
        errors.extend(_evidence_errors(evidence, repository_root=repository_root))
    ...
    if kind == "work-item" and report.get("status") == "DONE" and not evidence:
        errors.append("$.evidence: DONE work item requires evidence")
    elif kind == "work-item" and report.get("status") == "DONE":
        errors.extend(_evidence_errors(evidence, repository_root=repository_root))
    if kind == "work-item" and report.get("status") == "DONE" and report.get("blockers"):
        errors.append("$.blockers: DONE work item cannot contain blockers")
    if kind == "task-charter" and report.get("status") == "DONE" and not report.get("acceptance_criteria"):
        errors.append("$.acceptance_criteria: DONE Task Charter requires acceptance criteria")
    if kind == "scorecard":
