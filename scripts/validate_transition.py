...
EXCEPTION_STATES = {"BLOCKED", "FAILED", "NOT_VERIFIED"}
TERMINAL = {"CLOSED", "FAILED"}
LEGAL = {
    "INTAKE": {"KICKOFF", "BLOCKED", "FAILED", "NOT_VERIFIED"},
    "KICKOFF": {"PLANNING", "BLOCKED", "FAILED", "NOT_VERIFIED"},
    ...
    "NOT_VERIFIED": {"PLANNING", "EXECUTING", "EVIDENCE_REVIEW", "QA", "BLOCKED", "FAILED"},
}
...
def _require_evidence_review(state: dict) -> None:
...
def validate_transition(from_status: str, to_status: str, *, qa_verdict: str | None = None,
    ...
    if to_status in {"EVIDENCE_REVIEW", "BOSS_REVIEW", "CLOSED"} and not _readable_evidence(evidence):
